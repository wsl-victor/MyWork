...
#define MDJMainScreenWidth [UIScreen mainScreen].bounds.size.width
@interface SLScrollView()<UIScrollViewDelegate>
{
    int _x;
    CGFloat _x0;
}
...
-(void)addScroll1{
    ...
    self.scroll1 =[[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, MDJMainScreenWidth, self.xl_topHeight?self.xl_topHeight:50)];
    
    if (self.xl_buttonNames.count <=5) {
        self.scroll1.contentSize =CGSizeMake(MDJMainScreenWidth, 0);
    }else {
        self.scroll1.contentSize =CGSizeMake(MDJMainScreenWidth/5*self.xl_buttonNames.count, 0);
    }
    ...
    for (int i =0; i<self.xl_buttonNames.count; i++) {
        ...
        if (self.xl_buttonNames.count <=5) {
            temp.frame =CGRectMake(MDJMainScreenWidth/self.xl_buttonNames.count*i, 0, MDJMainScreenWidth/self.xl_buttonNames.count, self.xl_topHeight?self.xl_topHeight+ 10:50);
        }else {
            temp.frame =CGRectMake(MDJMainScreenWidth/5*i, 0, MDJMainScreenWidth/5, self.xl_topHeight?self.xl_topHeight + 10:50);
        }
    ...
    self.view2 =[[UIView alloc]initWithFrame:CGRectMake(_x0,CGRectGetMaxY(button0.frame)-(self.xl_buttonToSlider?self.xl_buttonToSlider:10) +36, size0.width, self.xl_sliderHeight?self.xl_sliderHeight:2)];
    
    self.view2 =[[UIView alloc]initWithFrame:CGRectMake(_x0,CGRectGetMaxY(button0.frame)-(self.xl_buttonToSlider?self.xl_buttonToSlider:100) + 36, size0.width, self.xl_sliderHeight?self.xl_sliderHeight:2)];
    ...
    grayLine.backgroundColor = [UIColor colorWithRed:234/255.0f green:234/255.0f  blue:234/255.0f  alpha:1.0f];
    grayLine.frame = CGRectMake(0, CGRectGetMaxY(button0.frame)-(self.xl_buttonToSlider?self.xl_buttonToSlider:100) + 38.3, MDJMainScreenWidth, 1);
    [self.scroll1 addSubview:grayLine];
...
-(void)addScroll2{
    self.scroll2 =[[UIScrollView alloc]initWithFrame:CGRectMake(0, self.xl_topHeight?self.xl_topHeight:50, MDJMainScreenWidth, self.frame.size.height -(self.xl_topHeight?self.xl_topHeight:50))];
    self.scroll2.backgroundColor = [UIColor whiteColor];
    self.scroll2.contentOffset=CGPointZero;
    self.scroll2.contentSize=CGSizeMake(MDJMainScreenWidth*self.xl_buttonNames.count, 0);
    ...
    self.scroll2.bounces = YES;
    self.scroll2.scrollsToTop =NO;
    
    for (int i =0; i<self.xl_buttonNames.count; i++) {
        
        if (!self.xl_views) {
            UIView *temp =[[UIView alloc]initWithFrame:(CGRect){{MDJMainScreenWidth*i, 0},self.scroll2.frame.size}];
            ...
            temp.frame =(CGRect){{MDJMainScreenWidth*i, 0},self.scroll2.frame.size};
            [self.scroll2 addSubview:temp];
        }
    }
...
-(void)changed:(UIButton *)button{
    ...
    if (self.xl_buttonNames.count <=5) {
        self.scroll2.contentOffset =CGPointMake(button.center.x*self.xl_buttonNames.count -MDJMainScreenWidth/2, 0);
    }else {
        self.scroll2.contentOffset =CGPointMake(button.center.x*5 -MDJMainScreenWidth/2, 0);
    }
...
-(void)scrollViewWillBeginDragging:(UIScrollView *)scrollView{
    _x =scrollView.contentOffset.x/MDJMainScreenWidth;
}
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    if (self.buttons.count >5) {
        UIButton *button =self.buttons[_x];
        int xAfter =scrollView.contentOffset.x/MDJMainScreenWidth;
        if (_x<xAfter) {
            
            if (_x>=2 && _x<=self.xl_buttonNames.count-4 ) {
                [UIView animateWithDuration:0.2 animations:^{
                    self.scroll1.contentOffset =CGPointMake(button.center.x -MDJMainScreenWidth/5*1.5, 0);
                }];
            }
            ...
            if (_x>=3 && _x<=self.xl_buttonNames.count-2 ) {
                [UIView animateWithDuration:0.2 animations:^{
                    self.scroll1.contentOffset =CGPointMake(button.center.x -MDJMainScreenWidth/5*3.5, 0);
                }];
            }
            if ((self.buttons.count ==6)|7 &&_x<3) {
                if ((_x ==1)|2) {
